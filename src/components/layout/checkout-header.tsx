"use client";

import Link from "next/link";
import Image from "next/image";
import { Lock, ShoppingBag } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useCart } from "@/context/cart-context";

/**
 * The header checkout gets instead of the site one.
 *
 * The full header carries a hamburger, a home link, Menu, Our story and
 * Instagram; the footer beneath it repeats most of that. That is eleven
 * ways out of a page whose only job is one form, and two of them lead to
 * Instagram. What survives here is the cart — which on this page is the
 * only way to fix a wrong quantity, so it is a tool rather than an exit —
 * and the logo, which points back to the menu rather than the homepage
 * because "leave checkout" almost always means "I need another item".
 */
export function CheckoutHeader() {
  const { itemCount, openCart } = useCart();

  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/80">
      <div className="mx-auto flex h-14 max-w-4xl items-center justify-between gap-3 px-4 sm:px-6">
        <Link href="/#menu" className="flex items-center gap-2.5">
          <Image
            src="/brand/mark-black.png"
            alt=""
            width={26}
            height={26}
            priority
            className="shrink-0"
          />
          <span className="font-heading text-lg font-semibold tracking-tight text-foreground">
            Your Neighbour
          </span>
        </Link>

        <div className="flex items-center gap-2">
          {/* The claim the page already makes in prose, in the form people
              actually register — and at the top, where it frames the form
              rather than arriving after they have committed. */}
          <span className="hidden items-center gap-1.5 text-xs font-medium text-muted-foreground sm:flex">
            <Lock className="size-3.5" aria-hidden="true" />
            Secure checkout
          </span>

          <Button
            variant="outline"
            size="icon"
            className="relative"
            aria-label={`Edit your order, ${itemCount} item${itemCount === 1 ? "" : "s"}`}
            onClick={openCart}
          >
            <ShoppingBag />
            {itemCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 flex size-5 items-center justify-center rounded-full bg-primary text-[11px] font-medium text-primary-foreground">
                {itemCount}
              </span>
            )}
          </Button>
        </div>
      </div>
    </header>
  );
}
